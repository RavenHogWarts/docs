...
## 提示词示例
提示词采用[变量](../advanced-usage/variables)语法 <code v-pre>{{"your prompt"}}</code>。您可以将此语法与任何自然语言查询结合使用，例如：
...
提示的响应结果可以通过[过滤器](../advanced-usage/filters)进一步处理。过滤器在从模型接收到提示响应后进行加工。例如：<code v-pre>{{"a summary of the page"|blockquote}}</code> 会将响应转换为块引用。
...
4. 将[提示词变量](../advanced-usage/variables.md#提示词变量)添加到您的[模板](../guide/templates)中。
...
您可以在解释器高级设置中覆盖默认上下文，并为每个[模板](../guide/templates)单独定义上下文。
...
这仅会在网页存在 `#main` 元素时运行解释器。[HTML 处理过滤器](../advanced-usage/filters.md#html-处理)(如 [`remove_html`](../advanced-usage/filters.md#remove_html)、[`strip_tags`](../advanced-usage/filters.md#strip_tags)和 [`strip_attr`](../advanced-usage/filters.md#strip_attr) )有助于进一步缩减上下文长度并提升处理速度。
...
- 在模板中使用 [`上下文`](#上下文) 字段提供页面上更具针对性的部分，或使用[过滤器](../advanced-usage/filters)（例如 <code v-pre>{{content|slice:0,1000}}</code> ）来裁剪上下文。