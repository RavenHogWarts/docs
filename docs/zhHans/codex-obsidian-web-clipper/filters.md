...
# 过滤器
过滤器允许您修改 Web Clipper [模板](./templates.md)中的[变量](./variables.md)。使用 <code v-pre>{{variable|filter}}</code> 语法将过滤器应用于变量。
- 过滤器适用于任何类型的[变量](./variables.md)，包括 `prompt`、`meta` 、 `selector` 和 `schema ` 变量。
...
## HTML 处理
处理 HTML 内容并将其转换为 Markdown 格式。请注意，您的输入[变量](./variables.md)必须包含 HTML 内容，例如使用 <code v-pre>{{fullHtml}}</code>、<code v-pre>{{contentHtml}}</code> 或 <code v-pre>{{selectorHtml:}}</code> 变量。
