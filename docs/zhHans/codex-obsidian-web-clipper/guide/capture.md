...
# 网页剪藏
安装 [Web Clipper](/codex-obsidian-web-clipper/) 浏览器扩展后，您可以通过多种方式访问它，具体取决于您的浏览器：
...
## 捕获网页
当你打开扩展程序时，Web Clipper 会根据模板中的设置从当前网页提取数据。你可以创建自己的[模板](/codex-obsidian-web-clipper/guide/templates)，并使用[变量](/codex-obsidian-web-clipper/advanced-usage/variables)和[过滤器](/codex-obsidian-web-clipper/advanced-usage/filters)自定义输出内容。
...
- 若存在[高亮标注](/codex-obsidian-web-clipper/guide/highlight)，系统将采用高亮部分内容。
...
1. `标题`栏可在此处切换模板、开启[高亮显示](/codex-obsidian-web-clipper/guide/highlight)并访问设置。
2. `属性`面板显示从页面提取的元数据，这些数据将作为属性保存在 Obsidian 中。
3. `笔记内容`将保存至 Obsidian。
4. `页脚`栏可选择知识库与文件夹，并添加至 Obsidian。

### 标题栏功能
- `模板`下拉菜单，用于在 Web Clipper 设置中保存的[模板](/codex-obsidian-web-clipper/guide/templates)之间切换。
...
- `文件夹`字段，用于定义保存至哪个文件夹。
- `解释器`，用于在页面上运行[自然语言提示](/codex-obsidian-web-clipper/advanced-usage/interpreter)。