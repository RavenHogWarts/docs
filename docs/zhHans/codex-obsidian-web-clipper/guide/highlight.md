...
# 高亮标注
[Web Clipper](../index.md) 允许您在网页上高亮显示文本，并选择想要保存到 Obsidian 中的元素。您的高亮内容会被保存，以便在返回页面时重新查看。

当您打开扩展程序时，可以[捕获](guide/capture.md)高亮内容并保存到 Obsidian。
...
通过 <code v-pre>{{content}}</code> [变量](advanced-usage/variables.md#预设变量)将高亮内容插入到已剪辑笔记中有三个选项：
